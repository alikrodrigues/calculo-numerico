...
export class BiseccaoLocaComponent implements OnInit, AfterContentInit {
  ...
  calcular() {
    ...
    let variavel4: number = Number(this.formGroup.controls['variavel4'].value);
    ...
    for(let i = 0; i < 8; i++) {
      ...
      let result: number = Math.pow((
        variavel * x), expo) - Math.pow((
          variavel2 * x), expo2) - (variavel3 * x) + variavel4;
